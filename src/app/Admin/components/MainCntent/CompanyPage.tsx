...
const CompanyAdmin = () => {
    ...
    const toggleDetail = (id: string,companyName:string, alamat:string, deadline:string, bidang:string, startDate: string, listPertanyaan:string[]) =>{
        handleCancel();
        setSelectedCompany({id, companyName, alamat, bidang, deadline,  startDate, listPertanyaan})
            ...
            <div className="mt-3 md:p-4 gap-2 md:gap-4 overflow-hidden pb-5 w-full h-full dark:bg-[#1e1f24] bg-[#ffffff] bg-border-2 border-[#232429] rounded-xl flex flex-col">
                ...
                <ul className=" text-sm md:text-base w-full overflow-y-scroll scrollbar-hide flex flex-col gap-2">
                    <li className="w-full text-[#62d28d]  font-semibold border-b-2 text-base md:text-lg border-black dark:border-white flex">
                        ...
                        <p className="w-[15%] text-center flex justify-center items-center ">Bidang</p>
                    ...
                    </li>
                    {
                        ...
                        <li key={index} className="w-full border-b-2 border-black dark:border-white flex">
                            ...
                            <p className="w-[15%] text-center flex justify-center items-center ">{company.bidang}</p>
                            ...
                            <div className="w-[10%] text-center flex justify-center items-center pb-1 ">
                                <button onClick={()=>{toggleDetail(company.id, company.name, company.alamat, company.deadline, company.bidang, company.startDate, company.listPertanyaan)}}  className=" bg-[#359be5] px-2 py-1 hover:bg-[#86c2ed] flex justify-center text-xs md:text-sm font-semibold items-center rounded-lg ">Detail</button>
                            </div>
                            <div className="w-[10%] text-center flex justify-center items-center pb-1 ">
